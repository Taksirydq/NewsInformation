...
from info.utils import pic_storage
from info.utils.response_code import RET
from . import admin_bp
from flask import render_template, request, session, redirect, url_for, current_app, jsonify, abort
...
def user_list():
    ...
    try:
        ...
        current_app.logger.error(e)
        return jsonify(errno=RET.DBERR, errmsg="查询数据有误")
...
def news_review_detail():
    ...
    3.逻辑处理
        3.1 根据news_id 查询出新闻对象
        3.2　通过: news.status = 0
        3.3 拒绝: news.status = -1, news.reason = 拒绝原因
    4.返回值
    """
...
def news_edit():
    ...
    news_dict_lsit = []
    for news in news_list if news_list else []:
        news_dict_lsit.append(news.to_review_dict())

    data = {
        "news_list": news_dict_lsit,
        "current_page": current_page,
        "total_page": total_page
    }
